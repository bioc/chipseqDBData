...
This package contains several ChIP-seq datasets for use in differential binding (DB) analyses in the `r Biocpkg("chipseqDB")` workflow [@lun2015reads]:

- H3K9ac ChIP-seq in murine pro-B and mature B cells [@domingo2012bcell]
- CREB binding protein (CBP) ChIP-seq in wild-type and CBP-knockout mouse embryonic fibroblasts [@kasper2014genomewide]

This vignette will briefly examine the processing statistics for these datasets.

# Examining the H3K9ac data

We load the data from `r Biocpkg("ExperimentHub")` using the `H3K9acData()` function.
...
We load the data from `r Biocpkg("ExperimentHub")` using the `CBPData()` function.
